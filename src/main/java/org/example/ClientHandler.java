...
public class ClientHandler {
    ...
    public void sendMessage(String msg) {
        try {
            System.out.println("SERVER: Отправить сообщение к " + name + ": " + msg);
            out.writeUTF(msg);
        } catch (IOException e) {
            e.printStackTrace();
            throw new RuntimeException(e);
    ...
    public void readMessages() {
        try {
            while (true) {
                ...
                server.broadcast(name + ": " + strFromClient); // если сообщение не начинается со слеша видят все участникам
            }
        } catch (IOException e) {
            e.printStackTrace();
            throw new RuntimeException(e);
